...
const RATIOS_LIST = [
  {
    ...
    desc: 'Характеризует способность компании мгновенно погашать краткосрочные обязательства за счет самых высоколиквидных активов — денежных средств и краткосрочных финансовых вложений.',
    colorKey: 0,
  ...
  },
  {
    ...
    desc: 'Показывает, достаточно ли у компании всех оборотных активов для погашения текущих краткосрочных обязательств в течение ближайшего года.',
    colorKey: 1,
    calculate: (struct: any, f: any) => ({
  ...
  {
    id: 'assets_cov',
    name: 'Обеспеченность обязательств активами',
    desc: 'Характеризует долю совокупных долгов, покрываемую всеми имеющимися активами организации. Отражает долгосрочную общую платежеспособность.',
    colorKey: 2,
    calculate: (struct: any, f: any) => ({
  ...
  {
    id: 'solvency_curr',
    name: 'Степень платежеспособности по текущим обяз.',
    desc: 'Показывает, сколько месяцев работы с текущей выручкой потребуется организации для полного погашения всей накопленной краткосрочной задолженности.',
    colorKey: 3,
    calculate: (struct: any, f: any) => ({
  ...
  },
  {
    ...
    desc: 'Показывает долю капитала собственников (собственных средств) в структуре общего баланса. Чем выше показатель, тем более независима организация от внешних кредиторов.',
    colorKey: 4,
    calculate: (struct: any, f: any) => ({
  ...
  },
  {
    ...
    desc: 'Показывает уровень обеспеченности собственными оборотными средствами. Демонстрирует, какая часть оборотных средств сформирована за счет собственного капитала компании.',
    colorKey: 5,
    calculate: (struct: any, f: any) => ({
  ...
  },
  {
    ...
    name: 'Отношение дебиторской задолженности к активам',
    desc: 'Отражает уровень отвлечения средств компании в расчеты с покупателями и дебиторами. Чрезмерно высокое значение указывает на угрозу кассовых разрывов.',
    colorKey: 6,
    calculate: (struct: any, f: any) => ({
  ...
  {
    id: 'leverage',
    name: 'Левередж ((1400+1500)/1300)',
    desc: 'Коэффициент финансового левереджа (заемный капитал / собственный). Отражает соотношение долгового и собственного финансирования и уровень долгового риска.',
    colorKey: 7,
    calculate: (struct: any, f: any) => ({
  ...
  },
  {
    ...
    desc: 'Показывает общую эффективность использования всех ресурсов компании. Выражает процент чистой прибыли, генерируемой каждым рублем совокупных активов.',
    colorKey: 12,
    calculate: (struct: any, f: any) => ({
  ...
  },
  {
    ...
    desc: 'Отражает долю операционной прибыли в каждом рубле полученной выручки компании. Указывает на эффективность ценообразования и сдерживания издержек.',
    colorKey: 13,
    calculate: (struct: any, f: any) => ({
  ...
  },
  {
    ...
    desc: 'Показывает, какой процент от общей полученной выручки остается у организации в качестве чистой прибыли после уплаты всех расходов, процентов и налога на прибыль.',
    colorKey: 14,
    calculate: (struct: any, f: any) => ({
